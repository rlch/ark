...
2. **macOS socket path overflow.** Supervisor's `ensure_session(&format!("ark-{}", spec.id.as_path_leaf()))` at orchestration.rs:182 produces ~49-char session names for bare ark — plus `/var/folders/.../T/zellij-501/contract_version_1/` (80 chars) = 129-byte socket path > 103 limit on macOS. Workaround today: `ZELLIJ_SOCKET_DIR=/tmp/zellij`. Follow-up: shorten the internal zellij session name (just `&spec.name` instead of the `ark-<full-leaf>` format).
