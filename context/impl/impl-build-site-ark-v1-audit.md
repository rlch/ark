...
| T-104 | Picker W3 New-agent form | PARTIAL | claude-code-first pivot | Form present in picker but `ark spawn` exec was replaced by bare-`ark` invocation. The kit wording "Enter submits via `run_command` exec of `ark spawn ...`" no longer matches the CLI surface; picker today execs `ark` (no spawn verb). Functionally equivalent — form still works — but the exact exec command changed. Marked PARTIAL because the CLI contract it relies on shifted. |
