...
| T-038 | 7 | tests R2 | DONE | `07c593b` | `[[bin]] ark-stub-ext` in `ark-ext-test-support`; wraps R1 `StubExtension` in `NdjsonServer::serve` pump over stdin/stdout; env-var config surface (`ARK_STUB_{VERSION,CAPABILITIES,INTENTS,VIEW_TYPES,METHODS,METHOD_NOT_FOUND_LIST}`) mirrors StubBuilder 5 axes; parity test `tests/subprocess_parity.rs` scripts 4 calls (pane/emit, pane/close, on_session_end, stack/clear-unimpl) against both in-process stub + spawned bin, asserts identical serialized result payloads + JSON-RPC `-32601` for the unimpl case; clean EOF→exit-0 shutdown gated via `child.wait()`; `CARGO_BIN_EXE_*` env not reliably set on this cargo 1.93 combo so test falls back to resolving `target/<profile>/ark-stub-ext` via `current_exe()` two-parents-up |
| T-039 | 7 | tests R3 | DONE | `78408b3` | version-mismatch matrix — 5 cells (1.1↔1.1 OK/no-warn, 1.1↔1.0 OK/no-warn, 1.0↔1.1 OK/WARN, 2.0↔1.1 UnsupportedVersion + call_log empty, 1.1↔2.0 symmetric reject). New test `crates/ark-ext-test-support/tests/version_mismatch.rs`. Wiring: (a) `StubExtension` got `initialize` override so `with_protocol_version` actually surfaces in the response (default trait body was method_not_found — blocked gate); (b) `ExtensionClient::handshake` now emits `tracing::warn!(target="ark.ext_proto.handshake", …)` when ext MINOR > client MINOR; ark-ext-proto gets `tracing` workspace dep. Existing `UnsupportedVersion` error path + `ExtensionError::unsupported_version` helper untouched. Capture layer is thread-scoped `with_default` + `futures::block_on` (tokio block_on swaps dispatcher). `cargo test -p ark-ext-test-support --test version_mismatch` 5/5 green; workspace lib tests unchanged at 1807. |
| T-040 | 7 | tests R4 | DONE | `e7120a1` | capability-gate matrix; 4 cells in `crates/supervisor/tests/capability_gate_matrix.rs` (548 LOC). Cells: (a) advertised+called ok; (b) not-advertised skipped, 0 stub calls + 0 serialized bytes via `GatedClient` wrapper + byte-counter AtomicUsize; (c) advertised-but-unimplemented → host logs WARN with method+cap, opt-out sticks (second `should_dispatch` returns false without RPC), session survives; (d) removed-in-MAJOR → `#[ignore]` per decision #4c (no real removal yet). Tracing capture via `tracing-subscriber` Registry+custom Layer. Dev-deps added to supervisor Cargo.toml: ark-ext-test-support, ark-ext-proto, ark-view, futures. Tests only — no production-code changes; F-015 opt-out path already landed in T-029. 3 pass + 1 ignore (green). |
| T-042 | 7 | tests R6+R7 | DONE | `9ca91a3` | integration suite; 10 tests split across 2 new files. R6 in `crates/supervisor/tests/manifest_intent_integration.rs` (4 tests): `manifest_intent_appears_in_registry` drives `ark_scene::ext::ExtensionRegistry::activate` + asserts `intent_names()` contains `stub.hello`; `scene_op_dispatches_to_manifest_intent` captures `intent/dispatch` req via StubExtension; `intent_register_rpc_method_is_gone` greps `crates/ark-ext-proto/src/lib.rs` for `"intent/register"` literal + `fn intent_register(`; `undeclared_intent_scene_op_rejected_at_compile` verifies trybuild fixture + golden + harness wiring (the golden is exercised by `cargo test -p ark-scene --test view_types_trybuild`). R7 in `crates/supervisor/tests/suppression_invalidation.rs` (6 tests): user_close records+broadcasts; same-params→Skip; new-params→EvictAndSpawn+post-evict Spawn; pane/emit after invalidation→HandleGone; drop→fresh map=empty; stack-child close skips write but broadcasts. Small scope bump: (a) scene-macros case (5) `check_intent` validator — walks `intent "name"` nodes, validates against `ark.core.*` + manifest-declared intents (parser extended to pick up `intents { intent "name" }` children); (b) new trybuild fixture `crates/scene/tests/ui/undeclared_intent_reference.{rs,stderr}` + harness update; (c) public-API widening: `ExtensionRegistry::intent_names() -> Vec<&str>` + `StubExtension::intent_dispatch` override. Validation: 10/10 R6+R7 + 15/15 test-support + 2/2 scene trybuild + 597/597 scene tests + 1807/1807 workspace lib. |
| T-043 | 8 | ext-surface R8 | DONE | `43c24d3` | bumped `CURRENT_PROTOCOL_VERSION::new(1,0) → ::new(1,1)` at `crates/ark-ext-proto/src/lib.rs:297`; doc-comment now enumerates 1.1 additions (6 RPC + 6 hooks + 8-flag caps + `-32006 ext-proto/handle-gone`); zero `::new(1, 0)` residuals for the const; 11 `"1.0"` / `::new(1, 0)` greps across crates inspected — all are explicit test-fixture versions (T-039 matrix cells, conformance stub, scene error test, doctor check — not CURRENT_PROTOCOL_VERSION-driven), none needed updates; workspace 2062 P / 19 ignored / 0 F; T-039 `version_mismatch` matrix 5/5 green under new const (tests drive versions explicitly so cells unshifted). Clears F-002. |
| T-044 | 8 | tests R8 | DONE | `<pending>` | CI gate — existing `.github/workflows/ci.yml` updated: clippy runs with `--locked` + documented `-A` allowlist (14 lints, all architectural/cosmetic drift — see workflow header for rationale); `cargo build --workspace --all-targets --locked`; `cargo test --workspace --locked --tests -- --test-threads=1` matches Phase-2 R8 acceptance ("no features, no env vars"). Local validation: fmt ✓ / clippy ✓ (0 errors, 9 warnings) / build ✓ / test ✓ (2057 pass, 4 ignored, 0 fail across 62 suites, 17.84s). Small code-hygiene fixes landed alongside: fmt drift swept workspace-wide (141 files, separate commit); 19 clippy-fix edits across 15 files (io_other_error / unwrap_or_default / doc-comment re-indent / manual_contains / inspect_err / sort_by_key / vec_init_then_push / single_match / assert-eq-literal-bool / match_like_matches_macro / etc.); 5 `#[allow(dead_code)]` for genuine test-only helpers (list::render_table, list::cwd_for, kill::ulid_a/b, view tests marker-impl structs). Workspace test baseline preserved. Followup: cutting the 14-lint `-A` allowlist at a future hygiene pass (tracked as tech debt in workflow comment). |
