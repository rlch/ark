...
| T-043 | 8 | ext-surface R8 | DONE | `<pending>` | bumped `CURRENT_PROTOCOL_VERSION::new(1,0) → ::new(1,1)` at `crates/ark-ext-proto/src/lib.rs:297`; doc-comment now enumerates 1.1 additions (6 RPC + 6 hooks + 8-flag caps + `-32006 ext-proto/handle-gone`); zero `::new(1, 0)` residuals for the const; 11 `"1.0"` / `::new(1, 0)` greps across crates inspected — all are explicit test-fixture versions (T-039 matrix cells, conformance stub, scene error test, doctor check — not CURRENT_PROTOCOL_VERSION-driven), none needed updates; workspace 2062 P / 19 ignored / 0 F; T-039 `version_mismatch` matrix 5/5 green under new const (tests drive versions explicitly so cells unshifted). Clears F-002. |
| T-044 | 8 | various | PENDING | — | CI gate |
