...
|------|-------|--------|-----|-------|
| #6 | Full-zellij PTY harness for claude-code-ext | PARTIAL (MVP scaffold) | `2c43879` | New crate `crates/ark-test-harness/` — reusable PTY-backed `ArkHarness` builder that lifts the logic from `crates/cli/tests/launch_pty.rs` into a library. Public API: `ArkHarness::try_new` (SKIP-branch via `Ok(None)` when zellij missing or nested), `HarnessBuilder::{new, session_name, with_claude_mock, extra_env, pty_size, build}`, `wait_for_ready`, `send_input`, `dump_screen`, `wait_for_text`, `shutdown`, `pty`, `pty_buffer`, `session_name`. Helpers: `stage_claude_shim` (tempdir + `claude` symlink → mock bin on unix; copy on non-unix), `MINIMAL_SCENE_KDL` / `write_minimal_scene`, `zellij_on_path`, `inside_zellij`, `locate_zellij`, `run_zellij`, `wait_for_session`, `kill_session`, `dump_screen`, `harness_can_run`, `discover_workspace_binary`. `PtyProcess` with `spawn`, `send_input`, `snapshot_bytes/lossy`, `kill`, `is_running`, `shutdown_with_timeout`; 256 KiB ring cap on the reader-thread buffer. Dev-dep only (portable-pty 0.8 = workspace pin; tempfile + anyhow + tracing = workspace deps). Own smoke test (`tests/smoke.rs`) covers: (a) `harness_can_run` primitive, (b) `Ok(None)` SKIP branch when environment unsuitable, (c) full launch + `wait_for_ready` + `shutdown` round-trip when zellij is on PATH outside of zellij AND `target/debug/ark` has been built. Binary discovery via workspace-root walk (`discover_workspace_binary`) since `CARGO_BIN_EXE_*` is package-scoped. 13 tests total (10 unit + 3 integration). Workspace 2345 → 2358 passing, 4 ignored. **Integration test with view-grid assertions deferred** — MVP covers scaffold + API surface + one smoke test; downstream crates can now add `ark-test-harness` as a dev-dep + resolve their own binaries to build richer scenarios (subagent burst, view rendering, `dump_screen` text assertions). |
