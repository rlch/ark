...
**Acceptance Criteria:**
- [ ] WIT package declared as `package ark:plugin@1.0.0;` and lives at `crates/ark-plugin-protocol/wit/plugin.wit` (additional files permitted in same directory; `wit-deps` resolution discouraged for v1). The `@1.0.0` tag is the "first stable WIT surface" marker — distinct from `ARK_ABI_VERSION` per R14 (WIT semver and binary ABI version move independently).
- [ ] Two worlds live in the shared package:
  - **`world plugin-base`** — the world PLUGIN AUTHORS extend. Imports the unconditional `ark:host/log`, `ark:host/clock`, `ark:host/plugin-id` trio; `use`s the shared `types` + `widget-tree-types` surface; exports the five lifecycle funcs. Does NOT import any `ark:cap/*` interface — plugin authors add cap imports in their OWN per-plugin world (which `include`s `plugin-base`).
  - **`world plugin-host`** — the MAXIMAL world the HOST binds against. `include`s `plugin-base` PLUS imports every `ark:cap/*` interface. Used by a single `wasmtime::component::bindgen!` invocation in `ark-host` so per-interface `add_to_linker` helpers exist for every cap; the host's `LinkerSet` conditionally registers each cap per variant (R4 coarse gate).
- [ ] Guest exports (in `plugin-base`) exactly: `on-install: func(reason: install-reason)`, `load: func()`, `update: func(event: update-event) -> bool`, `render: func(view-id: string, w: u32, h: u32) -> result<widget-tree, plugin-error>`, `pipe: func(message: pipe-message) -> bool`. Adding/removing exports = breaking change requiring world bump (and `ARK_ABI_VERSION` bump per R14).
...
- [ ] Guest bindings: `wit-bindgen` invocation produces compilable Rust output for the example plugin in `crates/ark-plugin-protocol/examples/echo/`. Plugin authors write per-plugin worlds in their own `wit/` directory (canonical `wit/deps/ark-plugin/` layout for the shared package) — see `examples/echo/wit/echo.wit` for the reference shape.
- [ ] Host bindings: a single `wasmtime::component::bindgen!({ path: "wit", world: "plugin-host", imports: { default: async | trappable } })` invocation in `ark-host` produces the host trait scaffolding for the union world. `with:` ascribes Rust types for any host-owned resources (Cluster 3 §3.4).
...
- [ ] **`ARK_ABI_VERSION` vs WIT package semver are DISTINCT.** The WIT package version (e.g. `package ark:plugin@1.0.0;` in `crates/ark-plugin-protocol/wit/plugin.wit`) is a WIT-level semver string used by `wasmtime::component::bindgen!` to pin the generated interface names and by downstream plugin authors' `include`/`import` directives. `ARK_ABI_VERSION` is the host↔plugin BINARY protocol version the inspect phase cross-checks against `ark-meta:v1`'s `ark-abi-version` field. They MOVE INDEPENDENTLY: a WIT minor/patch bump (e.g. `1.0.0 → 1.1.0` when adding an additive `ark:cap/*` interface) does NOT bump `ARK_ABI_VERSION`; only the triggers enumerated above in this requirement bump `ARK_ABI_VERSION`. Conversely, an `ARK_ABI_VERSION` bump MAY coincide with a WIT MAJOR bump when the cause is one of the R14 triggers that also changes the WIT world (e.g. a new required lifecycle export adds a WIT world export AND triggers `ARK_ABI_VERSION++`). A WIT patch bump for a pure doc-comment fix or a non-surface reformat is ALWAYS a no-op for `ARK_ABI_VERSION`.
