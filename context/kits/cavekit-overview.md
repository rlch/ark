...
| Scene + Extensions (v3) | cavekit-scene.md | 17 (R1-R17) | CONVERGED | v3 redesign: ark-native layout DSL (row/col/stack/span/@handle/mode/when=), views replace plugins, typed view-parametric handles `Pane<V>`+`Stack<V>` (2026-04-18 revision), Rhai expression-only mode (CEL + minijinja both dead, 2026-04-16), extensions unified (3 delivery modes: compiled-in/subprocess/wasm-component), reconciler via override-layout, composition via include-only, code-generated manifest via Rust derives. R10 wasm delivery mode SUPERSEDED 2026-04-20 by cavekit-plugin-protocol.md (ark-native wasm-component, not zellij plugin). See cavekit-scene.md changelogs. Build site needs regeneration (`/ck:map`). |
| Plugin protocol (wasm-component) | cavekit-plugin-protocol.md | 14 (R1-R14) | DRAFT | ark-native wasm-component plugin protocol replacing the embedded-metadata-via-facet-kdl model. Ark-host owns wasmtime + WIT-defined ABI + capability declaration via `ark:cap/*` imports + custom-section cross-check + per-cap linker variants + 5 lifecycle hooks (no deactivate) + 3-phase loader (inspect → compile → instantiate) + abstract widget tree (data not pixels) + bare `.wasm` distribution + `ark ext dev <dir>` + ABI versioning. Supersedes scene R10 wasm branch; amends distribution R3; replaces claude-code's control_verbs with on-install lifecycle. Built on Cluster 1 (Zed wasm host), Cluster 3 (wasmtime embedder), Cluster 4 (capability mechanics) of `context/refs/plugin-protocol-survey.md`. |
