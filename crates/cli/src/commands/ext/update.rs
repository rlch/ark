...
pub fn update_one(
    ...
    let install_meta = read_install_metadata(&install_dir)
        .map_err(|e| format!("reading .ark-install: {e}"))?;
...
fn read_installed_metadata(dir: &Path) -> Result<ExtensionMetadata, String> {
    ...
    parse_extension_metadata_kdl(&text)
...
"#
    ...
    fn update_one_refetches_from_path_source() {
        ...
        let outcome =
            update_one(&extensions_root, "picker", true).expect("update should succeed");
        ...
        let dotfile = fs::read_to_string(
            extensions_root.join("picker").join(".ark-install"),
        )
        .unwrap();
    ...
    fn update_one_fails_when_ark_install_missing_source() {
        ...
        fs::create_dir_all(&dir).unwrap();
        fs::write(dir.join("extension.kdl"), sample_manifest("broken", "0.1.0"))
            .unwrap();
    ...
    fn run_bulk_updates_every_installed_extension() {
        ...
        seed_installed(&extensions_root, "a", "0.1.0", &format!("path:{}", src_a.display()));
        seed_installed(&extensions_root, "b", "0.1.0", &format!("path:{}", src_b.display()));
        ...
        let a_meta = fs::read_to_string(
            extensions_root.join("a").join("extension.kdl"),
        )
        .unwrap();
        assert!(a_meta.contains("0.2.0"));
        let b_meta = fs::read_to_string(
            extensions_root.join("b").join("extension.kdl"),
        )
        .unwrap();
